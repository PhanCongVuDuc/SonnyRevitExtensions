...
public static class FaceExtensions
{
  ...
  {
    return faces.SelectMany( x => x.GetPoints() )
      .ToList() ;
  }
    ...
    Face face2 )
  {
    ...
    var list = curve.Tessellate() ;

    var listXyzes = new List<XYZ>() ;
  ...
  public static List<XYZ> GetIntersectionPoints( this Face face,
    List<Face> faces )
  {
    return faces.SelectMany( x => x.GetIntersectionPoints( face ) )
      .ToList() ;
  }
  ...
  public static List<XYZ> GetIntersectionPoints( this List<Face> faces1,
    List<Face> faces2 )
  {
    return faces1.SelectMany( x => x.GetIntersectionPoints( faces2 ) )
      .ToList() ;
  }
  ...
  public static List<XYZ> GetIntersectionPoints( this List<Face> faces,
    Line line )
  {
    return faces.SelectMany( x => x.GetIntersectionPoints( line ) )
      .ToList() ;
  }
  ...
  public static XYZ? GetOriginInFace( this Face face )
  {
    ...
    if ( face.IsInside( uv ) ) {
      ...
      for ( var i = 1 ; i < 99 ; i++ ) {
        ...
        var tempUv = new UV( uvTempFar.U * temp,
          uvTempFar.V * temp ) ;
  ...
  {
    return faces.SelectMany( x => x.GetCurves() )
      .ToList() ;
  }
